...
class DAFasterRCNN(GeneralizedRCNN):
    @configurable
    def __init__(
        self,
        *,
        ...
        self.DC_ins = DAInsHead(self.roi_heads.box_predictor.cls_score.in_features, self.levels).to(self.device)
    ...
    def forward(self, batched_inputs: Union[List[Dict], Tuple[List[Dict]]]):
        ...
        proposals_s, detector_losses, box_features_s = self.roi_heads(
            images_s,
            img_features_s,
            proposals_rpn_s,
        ...
        if batched_inputs_t is not None:
            ...
            proposals_t, _, box_features_t = self.roi_heads(
                images_t,
                img_features_t,
                proposals_rpn_t
            )
            ...
            loss_DC_consistency_s = self.consistency_loss(img_features_s, box_features_s, proposals_s, level_assignments_s)

            # Target consistency loss
            loss_DC_consistency_t = self.consistency_loss(img_features_t, box_features_t, proposals_t, level_assignments_t)
    ...
    def instance_dc_loss(self, box_features, level_assignments, domain_label):
        ...
        box_features_reversed = gradient_scalar(box_features, -1.0*self.dc_ins_grl_weight)
    ...
    def consistency_loss(self, img_features, box_features, proposals, level_assignments):
        """Consistency loss between image-level and instance-level domain classifier predictions."""
        # print("CONSISTENCY LOSS START")
        ...
        # print("DC_img_out:", {level: DC_img_out[level].size() for level in self.levels})
        ...
        # print("DC_img_rois_probs:", DC_img_rois_probs.size())
        DC_img_rois_probs = F.avg_pool2d(DC_img_rois_probs, kernel_size=self.pooler_resolution, stride=self.pooler_resolution)
        # print("DC_img_rois_probs:", DC_img_rois_probs.size())

        box_features = gradient_scalar(box_features, self.dc_consistency_weight*self.dc_ins_grl_weight)
        DC_ins_out = self.DC_ins(box_features, levels=level_assignments)
        DC_ins_probs = DC_ins_out.sigmoid()
        # print("DC_ins_probs:", DC_ins_probs.size())
