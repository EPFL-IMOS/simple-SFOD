...
class DatasetMapperEnhance(DatasetMapper):
    """
    ...
    def __init__(self, cfg, is_train=True):
        ...
        #self.style_image = utils.read_image(cfg.STYLE.STYLE_IMAGE, format=self.img_format)
        #self.style_transfer = StyleTransfer(cfg.STYLE.VGG_MODEL, cfg.STYLE.DECODER, self.style_image)
    ...
    def __call__(self, dataset_dict):
        ...
        #style_transfered_image = self.style_transfer(copy.deepcopy(image))
        ...
        #aug_input_style = T.StandardAugInput(style_transfered_image)
        #transforms_style = aug_input_style.apply_augmentations(self.augmentation)
        #image_weak_aug_style = aug_input_style.image
        ...
        '''
        image_pil = Image.fromarray(image_weak_aug_style.astype("uint8"), "RGB")
        ...
        #image_pil = Image.fromarray(image_weak_aug_style.astype("uint8"), "RGB")
        #image_strong_aug = np.array(self.strong_augmentation(image_pil))
        image_strong_aug = copy.deepcopy(image_weak_aug)
        dataset_dict["image"] = torch.as_tensor(
            np.ascontiguousarray(image_strong_aug.transpose(2, 0, 1))
        )
