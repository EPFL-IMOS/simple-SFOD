...
def convert_cityscapes_instance_only(data_dir, out_dir, file_name_suffix):
    ...
    json_name = f'instancesonly_filtered_%s{file_name_suffix}.json'
            ...
            for filename in files:
                if filename.endswith(polygon_json_file_ending):
                    ...
                    image['file_name'] = os.path.join("leftImg8bit",
                        data_set.split("/")[-1],
                        filename.split('_')[0],
                        filename.replace("_gtFine_polygons.json", f'_leftImg8bit{file_name_suffix}.png'))
...
def parse_args():
    ...
    parser.add_argument('--outdir', help="output dir for json files", default='datasets/cityscapes/annotations', type=str)
    parser.add_argument('--datadir', help="data dir for annotations to be converted", default="datasets/cityscapes", type=str)
    parser.add_argument('--file_name_suffix', help="suffix added to file_name (for foggy cityscapes)", default="", type=str)
    if len(sys.argv) == 1:
        parser.print_help()
        sys.exit(1)
...
if __name__ == '__main__':
    ...
    if args.dataset == "cityscapes":
        convert_cityscapes_instance_only(args.datadir, args.outdir, args.file_name_suffix)
    else:
